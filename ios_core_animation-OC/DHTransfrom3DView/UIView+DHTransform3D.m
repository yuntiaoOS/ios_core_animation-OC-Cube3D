...
- (void)setTransform3DWithPanTranslation:(CGPoint)panTranslation withView:(UIView*)sourceView
{
    // 我们要的向量是手指上次所在的点到这次所在的点连成的一个向量，这是你这次手指滑动的方向，传给transform3DRotate函数的向量是垂直于这个向量的向量。而我们已知的只有这个transition，也就是手指最开始的点到手指当前点连成的一个向量（也就是手指的位移，只考虑起始点和结束点）。
    // 画出图来就发现，我们要的向量就是当前向量-上一次手指的位移向量（向量减法）
    ...
    CGFloat radian = sourceView.transformUnit / maxTranslate_ * maxRotateRadian_;
    ...
    // 把旋转向量传给函数
    sourceView.layer.sublayerTransform = CATransform3DRotate(sourceView.layer.sublayerTransform, radian, rotateVector.endPoint.x,  rotateVector.endPoint.y, 0);
